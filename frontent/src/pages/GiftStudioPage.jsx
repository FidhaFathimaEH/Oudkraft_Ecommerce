...
const engravingOptions = ['No engraving', 'Initials', 'Name', 'Special date'];
const greetingCardOptions = ['Classic card', 'Luxury card', 'Handwritten note'];
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-[28px] border border-[#e3d9c4] bg-white p-6 shadow-sm">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                <label className="mb-2 block text-sm font-semibold uppercase tracking-[0.25em] text-[#C6A15B]">Ribbon color</label>
                <select value={ribbonColor} onChange={(e) => setRibbonColor(e.target.value)} className="w-full rounded-full border border-[#e3d9c4] bg-[#F8F4EC] px-4 py-3 outline-none">
                  {ribbonOptions.map((option) => <option key={option} value={option}>{option}</option>)}
                </select>
              </div>
              <div>
                <label className="mb-2 block text-sm font-semibold uppercase tracking-[0.25em] text-[#C6A15B]">Bottle engraving</label>
                <select value={engraving} onChange={(e) => setEngraving(e.target.value)} className="w-full rounded-full border border-[#e3d9c4] bg-[#F8F4EC] px-4 py-3 outline-none">
                  {engravingOptions.map((option) => <option key={option} value={option}>{option}</option>)}
                </select>
              </div>
              <div>
                <label className="mb-2 block text-sm font-semibold uppercase tracking-[0.25em] text-[#C6A15B]">Greeting card</label>
                <select value={greetingCard} onChange={(e) => setGreetingCard(e.target.value)} className="w-full rounded-full border border-[#e3d9c4] bg-[#F8F4EC] px-4 py-3 outline-none">
                  {greetingCardOptions.map((option) => <option key={option} value={option}>{option}</option>)}
                </select>
              </div>
              <div>
          ...
          <div className="rounded-[28px] border border-[#e3d9c4] bg-[#123F34] p-6 text-white shadow-sm">
            ...
            <div className="mt-6 space-y-4 text-sm text-[#efe4d0]">
              ...
              <div className="flex justify-between"><span>Ribbon</span><span>{ribbonColor}</span></div>
              <div className="flex justify-between"><span>Engraving</span><span>{engraving}</span></div>
              <div className="flex justify-between"><span>Card</span><span>{greetingCard}</span></div>
