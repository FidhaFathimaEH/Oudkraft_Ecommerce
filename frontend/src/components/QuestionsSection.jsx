...
export const QuestionsSection = ({ product }) => {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
      <div className="rounded-[32px] border border-[#e3d9c4] bg-[#fbf7f0] p-4 shadow-sm sm:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#C6A15B]">Customer questions</p>
            <h2 className="mt-3 font-serif text-2xl text-[#17362c] sm:text-3xl">Helpful answers before you buy</h2>
          </div>
        ...
        <div className="mt-8 space-y-4">
          {filteredQuestions.map((item, index) => (
            <motion.article key={`${item.question}-${index}`} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} className="rounded-[24px] border border-[#e3d9c4] bg-white p-4 sm:p-5">
              <p className="font-semibold text-[#17362c]">{item.question}</p>
              <p className="mt-3 text-sm leading-7 text-[#5b5b5b]">{item.answer}</p>
              <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-[#5b5b5b] sm:gap-3 sm:text-sm">
